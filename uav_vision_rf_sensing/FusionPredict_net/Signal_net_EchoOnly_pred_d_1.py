...
from ..FusionPredict_net.functions import denormalize_azimuth, set_INOUT_data, PositionalEncoding, \
    set_INOUT_data_someErrorInEcho, denormalize_distance
...
class PredModel(nn.Module):
    ...
    def forward(self, echo_ori):
        ...
        out = self.out_head(hist_feat)  # [B, 2]

        return out

    def pred_loss(self, output, labels):
        # [B,3]
        loss = F.mse_loss(output, labels[:,2:3])
        return loss

    def denormalize_output(self,x):
        d = denormalize_distance(x[:,2]).unsqueeze(1)
        return d
...
def train_model(right_model, model, train_loader, test_loader, device, save_path, epochs=10, lr=1e-3):
    ...
    log_dir = f"runs/Net_pred_ab_Echo/EchoOnly_est_d_{timestamp}"
...
if __name__ == "__main__":
    ...
    model_path = 'weights_P6/echo_net_pred_d_e100_1_snr0.pth'
    epoch = 100
