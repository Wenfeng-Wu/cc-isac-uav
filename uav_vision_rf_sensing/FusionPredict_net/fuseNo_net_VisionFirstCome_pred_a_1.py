...
from FusionPredict_net.functions import denormalize_azimuth, set_INOUT_data, FusionNet, \
...
class PredModel(nn.Module):
    ...
    #def __init__(self, input_dim=5, d_model=10, d_aux=6):
        """
                预测网络
                参数:
    ...
    def forward(self, vision_ori, echo_ori):
        ...
        fused_input = torch.cat((vision_ori[:, :-1, :], echo_ori[:, :-1, :]), dim=2)
        additional_input = vision_ori[:, -1, :]
...
def train_model(right_model, model, train_loader, test_loader, device, save_path, epochs=10, lr=1e-3):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i, batch in enumerate(train_loader, 1):
            ...
            output_pred = model.denormalize_output(output_pred)
            true_param = model.denormalize_output(output_data_true.squeeze(1))
            loss.backward()
            optimizer.step()
            ...
            if i % 10 == 0:
                ...
                true_az_out = true_param[0,0].detach().cpu().numpy()
...
if __name__ == "__main__":
    ...
    model_path = 'weights_P6/fuseNo_net_timeEst_a_e100_1_snr-1.pth'
    epoch = 100
